...
{   
      printk("S070WV20_MIPI_RGB++++LCD_open_flow+++kernel++\n");
...
static void LCD_panel_init(u32 sel)
{
 ...
 // if (SCIRPT_ITEM_VALUE_TYPE_STR != type) {  	
	...
	//panel_rst(1);
	//sunxi_lcd_delay_ms(100);
	//printk("++LCD_panel_init+++val.str:%s++\n",val.str);
	/*sunxi_lcd_dsi_clk_enable(sel);
	sunxi_lcd_delay_ms(50);
	for(i=0;;i++)
	{
		//if(!strcmp("S070WV20_MIPI_RGB",val.str)) {
		    printk("S070WV20_MIPI_RGB_panel[%d]++normal -+++\n",i);
	...
	sunxi_lcd_dsi_gen_write_1para(sel,0x6B,0x61);
