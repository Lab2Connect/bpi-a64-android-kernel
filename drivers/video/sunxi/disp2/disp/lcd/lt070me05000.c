...
#define te_en(val)  sunxi_lcd_gpio_set_value(sel, 2, val)
...
static void LCD_bl_open(u32 sel)
{
	printk("[BPI]LCD_bl_open\n");
	sunxi_lcd_backlight_enable(sel);//config lcd_bl_en pin to open lcd backlight
}

static void LCD_bl_close(u32 sel)
{
	printk("[BPI]LCD_bl_close\n");
...
static void LCD_panel_init(u32 sel)
{
	...
	printk("[BPI]LCD_panel_init, bright=%d\n", bright);
	...
	sunxi_lcd_dsi_gen_write_5para(sel, 0xde, 0x01, 0x04, 0x3f, 0xf0, 0x10);
...
//---------------------------------------------------------------------------------------------
	...
	sunxi_lcd_dsi_gen_write_5para(sel,0xb3,0x14,0x08,0x00,0x22,0x00);
	sunxi_lcd_dsi_gen_write_1para(sel,0xb4,0x0c);
	sunxi_lcd_dsi_gen_write_2para(sel,0xb6,0x3a,0xc3);
	...
	sunxi_lcd_dsi_dcs_write_1para(sel,0x53,0x2c);
	sunxi_lcd_dsi_dcs_write_1para(sel,0x3a,0x77);
	sunxi_lcd_dsi_dcs_write_4para(sel,0x2a,0x00,0x00,0x04,0xaf);
	sunxi_lcd_dsi_dcs_write_4para(sel,0x2b,0x00,0x00,0x07,0x7f);
	sunxi_lcd_dsi_dcs_write_0para(sel,DSI_DCS_SET_DISPLAY_ON);
...
{
	printk("[BPI]LCD_set_bright, bright=%d\n", bright);
	sunxi_lcd_dsi_dcs_write_1para(sel,0x51,bright);
